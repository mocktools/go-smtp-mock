...
// thread-safe set of server net.Listener
...
// thread-safe set of server port value
...
// thread-safe read of server port value
...
// thread-safe read to check if server has been started
...
// thread-safe setting of started flag to indicate server has been started
...
// thread-safe setting of started flag to indicate server has been stopped
...
// Returns copy of message data
func (server *Server) Messages() []Message {
	server.Lock()
	defer server.Unlock()
	copiedItems := []Message{}
	for i := 0; i < len(server.messages.items); i++ {
		copiedItems = append(copiedItems, *server.messages.items[i])
	}
	return copiedItems
}
